...
int main(int i_arg, char**)
{
	...
	HeapManagerProxy::CreateHeapManager(pHeapMemory, sizeHeap, numDescriptors);

	bool success = MemorySystem_UnitTest();
	assert(success);

	// Clean up your Memory System (HeapManager and FixedSizeAllocators)
	HeapManagerProxy::Destroy(HeapManagerProxy::pHeapManager);
...
bool MemorySystem_UnitTest()
{
	...
	do
	{
		...
		void* pPtr = Allocate::malloc(sizeAlloc);

		// if allocation failed see if garbage collecting will create a large enough block
		if (pPtr == nullptr)
		{
			HeapManagerProxy::Collect();

			pPtr = Allocate::malloc(sizeAlloc);
	...
	if (!AllocatedAddresses.empty())
	{
		...
		void* pPtr = Allocate::malloc(totalAllocated / 2);

		if (pPtr)
		{
			std::cout << "large test allocation has passed!" << std::endl;
			Allocate::free(pPtr);
		}
		else
		{
			// something failed
			std::cout << "large test allocation has failed!" << std::endl;
			return false;
		}
	}
	else
	...
	std::cout << "Unit test has passed!" << std::endl;
